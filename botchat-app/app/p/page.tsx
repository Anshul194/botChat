...
function BlockRenderer({ block, theme }: { block: PublicBlock; theme: any }) {
    ...
    switch (type) {
        ...
        case "hero_aesthetic_section":
            return (
                <div className="relative w-[calc(100%+3rem)] -mx-6 mt-0 mb-10 overflow-hidden shadow-2xl bg-black group min-h-[420px] flex items-center justify-center">
                    {settings.image ? (
                        <img src={settings.image} className="absolute inset-0 w-full h-full object-cover opacity-70 group-hover:scale-105 transition-transform duration-[2s]" />
                    ) : (
                    ...
                    <div className="relative z-10 p-10 w-full text-center flex flex-col items-center">
                        {(settings.brand_name || settings.headline) && (
                            <p className="text-[11px] font-black uppercase tracking-[0.4em] text-white/90 mb-4 drop-shadow-md">{settings.brand_name || settings.headline}</p>
                        )}
                        <h2 className="text-[42px] font-black leading-[1] tracking-tight text-white mb-6 drop-shadow-xl">
                            {settings.title || "Elevate Your Vision"}
                        </h2>
                        {(settings.subtitle || settings.subheadline || settings.description) && (
                            <p className="text-[16px] text-white/80 font-medium leading-relaxed max-w-[85%] mb-8 drop-shadow-md">
                                {settings.subtitle || settings.subheadline || settings.description}
                            </p>
                        )}
                        {settings.cta_text && (
                            <button className="px-10 py-4 rounded-full bg-white text-black text-[14px] font-black uppercase tracking-widest shadow-[0_10px_30px_rgba(255,255,255,0.3)] hover:scale-105 active:scale-95 transition-all">
                                {settings.cta_text}
                            </button>
                        )}
                    </div>
        ...
        case "stats_minimal_section":
            return (
                <div className="w-full mt-2 mb-6 py-6 border-y border-white/10">
                    {(settings.items || []).length > 0 ? (
                        <div className="grid grid-cols-3 gap-4 divide-x divide-white/10">
                            {(settings.items || []).slice(0, 3).map((s: any, i: number) => (
                                <div key={i} className="flex flex-col items-center text-center px-2">
                                    <span className="text-[24px] font-black tracking-tight" style={{ color: effectiveTextColor }}>{s.value || "—"}</span>
                                    <span className="text-[9px] font-bold uppercase tracking-widest opacity-40 mt-1" style={{ color: effectiveTextColor }}>{s.label}</span>
                                </div>
                            ))}
                        </div>
                    ) : null}
        ...
        case "header_profile_section":
            return (
                <div className="relative w-[calc(100%+3rem)] -mx-6 -mt-10 mb-10 overflow-hidden rounded-b-[48px] bg-white/5 backdrop-blur-md border-b border-white/10 pb-10 shadow-2xl">
                    {settings.cover_image ? (
                        <img src={settings.cover_image} className="w-full h-[200px] object-cover" />
                    ) : (
                        <div className="w-full h-[200px] bg-gradient-to-br from-indigo-500/30 via-purple-500/30 to-pink-500/30" />
                    )}
                    <div className="flex flex-col items-center text-center -mt-14 relative z-10 px-8">
                        {settings.avatar ? (
                            <div className="p-1.5 rounded-full bg-white/10 backdrop-blur-xl mb-4 shadow-2xl">
                                <img src={settings.avatar} className="w-28 h-28 rounded-full object-cover border-4 border-white dark:border-[#0a0a0a]" />
                            </div>
                        ) : (
                            <div className="w-28 h-28 rounded-full bg-white border-4 border-slate-100 flex items-center justify-center mb-4 shadow-xl"><User size={40} className="opacity-50" /></div>
                        )}
                        <p className="text-[28px] font-black tracking-tight mb-2 drop-shadow-sm" style={{ color: effectiveTextColor }}>{settings.name || "Profile Name"}</p>
                        {settings.bio && <p className="text-[15px] font-medium opacity-80 leading-relaxed max-w-[85%] drop-shadow-sm" style={{ color: effectiveTextColor }}>{settings.bio}</p>}
                    </div>
                </div>
            );
