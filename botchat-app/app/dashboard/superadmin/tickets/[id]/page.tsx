...
export default function SuperAdminTicketDetailPage() {
    ...
    const handleStatusChange = async (newStatus: "Resolved" | "Closed") => {
        if (!confirm(`Mark ticket ${ticket?.ticket_number} as ${newStatus}?`)) return;
    ...
    const handleDelete = async () => {
        if (!confirm(`Permanently delete ticket ${ticket?.ticket_number}? This cannot be undone.`)) return;
        setActionLoading(true);
        try {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-[var(--border)] pb-4">
                    <div>
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-xs px-2 py-0.5 rounded-md font-bold uppercase tracking-wider bg-slate-500/10 border border-slate-500/20 text-slate-600">
                                {ticket.category?.name || "General"}
                            </span>
                            <span className="text-xs px-2 py-0.5 rounded-md font-bold uppercase tracking-wider bg-purple-500/10 border border-purple-500/20 text-purple-600">
                                Tenant: {ticket.tenant_id || "Central"}
                            </span>
                        </div>
                        <h1 className="text-xl font-extrabold text-[var(--foreground)] mt-1.5">{ticket.subject}</h1>
                        <p className="text-xs text-[var(--muted-foreground)] mt-0.5">
                            Submitted by <span className="font-bold text-[var(--foreground)]">{ticket.user?.name}</span> ({ticket.user?.email})
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        {ticket.status !== "Resolved" && (
                            <button
                                ...
                                className="px-3.5 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold uppercase tracking-wider hover:bg-emerald-700 transition-all inline-flex items-center gap-1.5 shadow-sm"
                            >
                        ...
                        {ticket.status !== "Closed" && (
                            <button
                                ...
                                disabled={actionLoading}
                                className="px-3.5 py-2 rounded-xl bg-slate-700 text-white text-xs font-bold uppercase tracking-wider hover:bg-slate-800 transition-all inline-flex items-center gap-1.5 shadow-sm"
                            >
                        ...
                        <button
                            ...
                            disabled={actionLoading}
                            className="px-3.5 py-2 rounded-xl bg-red-600 text-white text-xs font-bold uppercase tracking-wider hover:bg-red-700 transition-all inline-flex items-center gap-1.5 shadow-sm"
                        >
                            ...
                            <div className="flex items-center gap-2 font-bold">
                                {msg.is_admin_reply ? (
                                    <>
                                        ...
                                        <span className="text-primary">Super Admin (You)</span>
                                    </>
                                ) : (
                                    <>
                                        <User className="w-4 h-4 text-[var(--muted-foreground)]" />
                                        <span className="text-[var(--foreground)]">{msg.user?.name || "Customer"}</span>
                                    </>
                                )}
                            </div>
            ...
            <form onSubmit={handleReply} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5 shadow-sm space-y-4">
                <h3 className="text-xs font-extrabold uppercase tracking-wider text-[var(--foreground)]">Official Admin Reply</h3>
                <textarea
                    rows={4}
