...
import { Search, MessageSquare, ArrowUpRight, Loader2, Filter, Settings } from "lucide-react";
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-extrabold tracking-tight text-[var(--foreground)]">Global Support Tickets</h1>
                    <p className="text-xs sm:text-sm text-[var(--muted-foreground)] mt-1">Super Admin dashboard for customer support and ticket resolutions across all tenants.</p>
                </div>
                <Link
                    href="/dashboard/superadmin/support-categories"
                    className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--card)] text-[var(--foreground)] font-bold text-xs uppercase tracking-wider hover:bg-[var(--muted)]/50 transition-all shadow-sm"
                >
                    <Settings className="w-4 h-4 text-primary" /> Manage Categories
                </Link>
            </div>
                                ...
                                {tickets.map((t) => (
                                    <tr key={t.id} className="hover:bg-[var(--muted)]/20 transition-colors">
                                        ...
                                        <td className="py-3.5 px-4">
                                            <div className="font-bold text-[var(--foreground)]">{t.user?.name || "User"}</div>
                                            <div className="text-[10px] text-[var(--muted-foreground)]">{t.tenant_id || "Central"} • {t.user?.email}</div>
                                        </td>
                                        ...
                                        <td className="py-3.5 px-4 text-right">
                                            <button
                                                onClick={() => router.push(`/dashboard/superadmin/tickets/${t.id}`)}
