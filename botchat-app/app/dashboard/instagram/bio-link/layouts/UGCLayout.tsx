...
export const UGCLayout = ({
    ...
    return (
        <div className="flex flex-col w-full min-h-full overflow-x-hidden" style={{ background: C.bg, color: C.dark, fontFamily: "'Inter', sans-serif" }}>
            ...
            <section className="px-4 pt-5 pb-4" style={{ background: C.hero }}>
                <div className="flex flex-col items-center text-center gap-3">
                    ...
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border"
                          style={{ borderColor: C.accent1, color: C.accent1, background: "#fdf4ff" }}>
                    ...
                    {/* Description */}
                    <p className="text-[11px] leading-relaxed font-medium max-w-[260px]"
                    ...
                    <div className="flex gap-2 flex-wrap justify-center">
                        {(h.buttons?.length ? h.buttons : [
                            ...
                            <a key={i} href={btn.link || "#"}
                               className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider active:scale-95 transition-all"
                               style={btn.style === "primary"
                                   ? { background: C.pricingHL, color:"#fff" }
                                   : { background:"#fff", border:`1.5px solid ${C.accent3}`, color: C.accent3 }}>
                                {btn.text}
                            </a>
                        ))}
                    </div>
                    ...
                    <div className="flex gap-2">
                        {[Instagram, Youtube, Mail, Share2].map((Icon, i) => (
                            <div key={i} className="w-8 h-8 rounded-full bg-white border flex items-center justify-center shadow-sm cursor-pointer active:scale-90 transition-all"
                                 style={{ borderColor: "#e9d5ff", color: C.accent3 }}>
                                <Icon size={13} />
                            </div>
                        ))}
                    </div>
            ...
            {/* ══ TESTIMONIAL ═══════════════════════════════ */}
            <section className="mx-3 my-3 rounded-2xl p-4 shadow-lg relative overflow-hidden text-white"
                     style={{ background: C.testiBg }}>
                ...
                <p className="text-[12px] font-medium italic leading-relaxed mb-3 relative z-10">
                    "{testiB?.settings?.quote || "Working together transformed our brand presence. The results were immediate and measurable."}"
                </p>
                <div className="flex items-center gap-2.5 relative z-10">
                    <div className="w-9 h-9 rounded-full border-2 border-white/40 overflow-hidden shrink-0">
                        {testiB?.settings?.author_image
                            ? <img src={testiB.settings.author_image} className="w-full h-full object-cover" />
                    ...
                    <div>
                        <p className="text-[11px] font-black tracking-wide">
                            {testiB?.settings?.author_name || "Sarah Chen"}
                        </p>
                        <p className="text-[9px] font-bold uppercase tracking-widest opacity-70">
                            {testiB?.settings?.author_role || "Founder & CEO"}
                        </p>
                    </div>
                </div>
            </section>
                ...
                <div className="space-y-2">
                    {faqs.map((faq: any, i: number) => (
                        <div key={i} className="rounded-xl overflow-hidden border"
                             style={{ borderColor:"#ede9fe", background:"#fff" }}>
                            <button
                                className="w-full flex items-center justify-between p-3 text-left"
                                onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                                <span className="text-[11px] font-black leading-snug pr-2" style={{ color: C.dark }}>
                                    {faq.question || faq.q}
                                </span>
                                <ChevronDown size={13} className="shrink-0 transition-transform duration-300"
                                    style={{ color: C.accent3, transform: openFaq === i ? "rotate(180deg)" : "rotate(0deg)" }} />
                            </button>
                            {openFaq === i && (
                                <div className="px-3 pb-3 pt-0 border-t" style={{ borderColor:"#ede9fe" }}>
                                    <p className="text-[10.5px] leading-relaxed pl-2.5 border-l-2" style={{ borderColor: C.accent3, color:`${C.dark}80` }}>
                                        {faq.answer || faq.a}
                                    </p>
                                </div>
                            )}
