    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-[#FDFDFF]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-6 rounded-2xl border border-slate-100 shadow-xs">
            ...
            <Field label="Select Instagram Account" required>
              <div className="relative">
                ...
                >
                  <option value="">{isLoadingPages ? "Syncing Accounts..." : "Select account"}</option>
                  {pages.map(p => (
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between px-1">
                  ...
                  <div className="flex gap-3 text-xs font-medium text-pink-500">
                    ...
                    <button onClick={() => window.location.href = "/dashboard/instagram/bot-replies"} className="hover:underline">+ Add Template</button>
                  </div>
                </div>
                <div className="relative">
                  <select 
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs grid grid-cols-1 gap-4">
            <div className="flex items-center justify-between py-1 px-1">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[13px] font-medium text-slate-600">Reply user multiple times?</span>
              </div>
          ...
          <div className="bg-white border border-slate-100 rounded-[22px] p-6 shadow-xs space-y-5">
            ...
            <div className="flex items-center gap-4 cursor-pointer group" onClick={() => setForm({ ...form, message_type: "filter" })}>
              ...
              <span className={cn("text-sm font-medium", form.message_type === "filter" ? "text-pink-600" : "text-slate-400")}>Filter by keyword</span>
            </div>
          </div>

          <div className="space-y-8">
            {form.message_type === "generic" ? (
              <div className="bg-white p-7 rounded-2xl border border-slate-100 shadow-sm space-y-8">
              ...
              <div className="space-y-8">
                ...
                <button onClick={handleAddRule} className="w-full py-4 rounded-xl border-2 border-dashed border-pink-200 text-pink-500 font-bold text-[12px] hover:bg-pink-50 hover:border-pink-300 transition-all flex items-center justify-center gap-2 uppercase tracking-widest">
        ...
        <div className="flex gap-4 p-8 bg-white border-t border-slate-100 flex-shrink-0">
          <button onClick={onClose} className="flex-1 py-3.5 rounded-xl border border-slate-200 text-slate-500 font-bold text-[13px] hover:bg-slate-50 transition-all uppercase">Cancel</button>
          <button onClick={handleSave} disabled={isSaving} className="flex-[2] py-3.5 rounded-xl bg-pink-600 text-white font-black text-[14px] shadow-xl shadow-pink-100 flex items-center justify-center gap-3 active:scale-95 transition-all disabled:opacity-50 tracking-widest uppercase">
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex items-center justify-center sticky top-0 z-40 shadow-xs">
        <div className="max-w-[1400px] w-full px-8 py-5 flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Instagram Automation</p>
                <h1 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mt-1">Reply Templates</h1>
            </div>
          </div>
          <button onClick={() => setFormModal({ open: true, mode: "create", template: null })} className="px-8 py-3.5 rounded-[22px] bg-pink-600 text-white font-black text-[12px] shadow-xl shadow-pink-100 active:scale-95 transition-all flex items-center gap-3 uppercase tracking-widest">
      ...
      <div className="max-w-[1400px] mx-auto px-8 py-10 space-y-8">
        <div className="flex items-center justify-between gap-6">
          <div className="px-6 py-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center gap-3 shadow-sm">
            ...
            <span className="text-[12px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">{templates.length} Active Nodes</span>
          </div>
          <div className="relative flex-1 max-w-md group">
            ...
            <input type="text" placeholder="Lookup Templates..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-14 pr-6 py-4 rounded-[22px] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 focus:border-pink-400 outline-none shadow-sm font-bold text-[14px] transition-all placeholder:text-slate-200"
            />
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="grid grid-cols-[2fr_1fr_1.5fr_120px] gap-8 px-10 py-6 bg-slate-50/50 dark:bg-slate-950/20 border-b border-slate-50 dark:border-slate-800">
          ...
          {isLoading ? (
            <div className="p-32 text-center flex flex-col items-center gap-6">
              <div className="w-16 h-16 rounded-[24px] bg-pink-50 flex items-center justify-center">
                 <RefreshCw className="w-8 h-8 animate-spin text-pink-400" />
              </div>
              <p className="text-[11px] font-black text-slate-300 uppercase tracking-widest">Compiling Assets...</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="p-32 text-center flex flex-col items-center">
              <div className="w-20 h-20 rounded-[32px] bg-slate-50 dark:bg-slate-900 flex items-center justify-center mb-8">
                <Settings className="w-10 h-10 text-slate-200 dark:text-slate-800" />
              </div>
              <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Empty Inventory</h3>
              <p className="text-sm text-slate-400 font-medium mt-3 italic uppercase tracking-wide">Register your first automation layer using the terminal above.</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
              {filtered.map(t => (
                <div key={t.id} className="grid grid-cols-[2fr_1fr_1.5fr_120px] gap-8 px-10 py-8 items-center hover:bg-slate-50/20 transition-all group border-b border-transparent last:border-0">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                        <span className="text-[15px] font-black text-slate-900 dark:text-white uppercase tracking-tight truncate block max-w-[280px]">{t.name}</span>
                        <div className="flex items-center gap-2 mt-1">
                             <Instagram size={12} className="text-pink-500" />
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">ID: {t.id}</span>
                        </div>
                    </div>
                  </div>
                  <div>
                    <span className={cn("px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest", t.reply_type === "filter" ? "bg-pink-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500")}>
                        {t.reply_type || 'Generic'}
                    </span>
                  </div>
                  <div className="flex flex-col gap-1">
                     <span className="text-[12px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Clock className="w-3.5 h-3.5" />
                        Live Sync
                     </span>
                     <span className="text-[11px] font-bold text-slate-300 uppercase pl-5">March 29, 2026</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <button onClick={() => handleEdit(t)} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-pink-600 hover:bg-pink-50 transition-all active:scale-90">
                      <Edit3 className="w-4 h-4" />
                    </button>
                    <button onClick={() => setDeleteId(t.id)} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all active:scale-90">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
