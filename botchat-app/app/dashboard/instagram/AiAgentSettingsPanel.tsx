...
function PinkToggle({ enabled, onToggle, label }: { enabled: boolean; onToggle: () => void; label: string }) {
    return (
        <div className="flex items-center justify-between p-5 bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 rounded-xl hover:border-pink-50 dark:hover:border-pink-900/20 transition-all group">
            ...
            <button
                ...
                className={cn(
                    ...
                    enabled ? "bg-pink-500 shadow-pink-500/20" : "bg-neutral-200 dark:bg-neutral-800"
                )}
            >
                <motion.div
                        ...
                        <div className="flex items-center gap-6">
                            <button
                                ...
                                className={cn(
                                    ...
                                    settings.ai_enabled ? "bg-pink-500 text-white shadow-lg shadow-pink-500/20" : "bg-neutral-100 text-neutral-400 dark:bg-neutral-800"
                                )}
                            >
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    settings.ai_enabled ? "bg-pink-500" : "bg-neutral-200 dark:bg-neutral-800"
                                )}
                            >
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
                                ...
                                <FormField label="Restricted Topics" hint="Add topics and press Enter to secure.">
                                    <div className="bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 rounded-2xl p-3 min-h-[56px] flex flex-wrap gap-2 shadow-sm focus-within:border-pink-200 transition-all">
                                        {topicList.map(topic => (
                                            <span key={topic} className="flex items-center gap-2 px-4 py-1.5 bg-pink-50 dark:bg-pink-500/10 text-pink-500 text-[11px] font-black uppercase tracking-widest rounded-xl border border-pink-100 dark:border-pink-900/30">
                                                {topic}
                ...
                <div className="space-y-10">
                    <FormField label="Neural Training Data">
                        <div className="flex items-center gap-8 absolute right-0 -top-8">
                            <button className="text-[11px] font-black text-pink-500 tracking-widest flex items-center gap-1.5 hover:opacity-70 transition-all"><Plus size={14} strokeWidth={3} /> NEW</button>
                            <button onClick={() => dispatch(fetchCampaigns())} className="text-[11px] font-black text-pink-500 tracking-widest flex items-center gap-1.5 hover:opacity-70 transition-all"><RefreshCw size={14} strokeWidth={3} /> REFRESH</button>
                        </div>
                        <div className="relative group">
                            <select
                                value={settings.ai_training_campaign_id || ""}
                                onChange={(e) => setSettings(s => ({ ...s, ai_training_campaign_id: e.target.value }))}
                                className="w-full h-14 px-6 appearance-none bg-neutral-50 dark:bg-neutral-800 border-none rounded-2xl text-sm font-bold text-neutral-700 dark:text-neutral-300 outline-none focus:ring-2 focus:ring-pink-500/20 transition-all cursor-pointer"
                            >
                                ...
                                <FormField label="Inference Delay (Seconds)" hint="Simulate natural conversation latency (Max 60s)">
                                    <div className="bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 rounded-2xl h-14 px-6 flex items-center shadow-sm">
                                        <input
                                            ...
                                            className="w-full bg-transparent outline-none text-xl font-black text-[#5850A4] dark:text-pink-400"
                                            placeholder="0"
                                        />
                                    </div>
                                </FormField>
