...
interface FullAccountReplyModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSaved: () => void;
    instagramId: string;
...
export function FullAccountReplyModal({
    isOpen,
    onClose,
    onSaved,
    instagramId,
    platform
}: FullAccountReplyModalProps) {
    ...
    const fetchConfig = async () => {
        setIsLoading(true);
        try {
            const res = await api.get(`/instagram/comment-manager/full-account-reply/${instagramId}?platform=instagram`);
            ...
            console.error("Fetch Full Config Error:", error);
            resetForm();
        } finally {
            setIsLoading(false);
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            let res;
            if (existingConfigId) {
                res = await api.put(`/instagram/comment-manager/full-account-reply/${existingConfigId}?platform=instagram`, payload);
            } else {
                res = await api.post(`/instagram/comment-manager/full-account-reply?platform=instagram`, payload);
            }
                    ...
                    <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 bg-white sticky top-0 z-20">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-[14px] font-semibold text-slate-800">
                                {existingConfigId ? 'Edit' : 'Create'} Full Account Reply Template (Instagram)
                            </h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-[#FDFDFF] custom-scrollbar">
                        ...
                        {isLoading ? (
                            <div className="h-[400px] flex flex-col items-center justify-center space-y-4">
                                ...
                                <p className="text-sm font-medium text-slate-400">Loading account configuration...</p>
                            </div>
                        ) : (
                            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                
                                <div className="bg-pink-50/50 border border-pink-100 rounded-xl px-6 py-3 mb-8">
                                    <p className="text-[12px] font-medium text-pink-600">Configure global automated responses for your entire account.</p>
                                </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-1 gap-8 bg-white p-6 rounded-2xl border border-slate-100 shadow-xs">
                                        <Field label="Automation Strategy Name" required icon={Edit3}>
                                            <input
                                                type="text"
                                                value={name}
                                                onChange={e => setName(e.target.value)}
                                                placeholder="e.g. Global Welcome Strategy"
                                    ...
                                    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <Field label="Offensive Keywords" desc="(comma separated)" icon={Filter}>
                                                <textarea
                                                    rows={4}
                                            ...
                                            <Field label="Private Reply Template (Offensive)" icon={RefreshCw}>
                                    ...
                                    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs grid grid-cols-1 gap-4">
                                        ...
                                        <div className="flex items-center justify-between py-1 border-b border-slate-50">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[13px] font-medium text-slate-600">Enable automation replies?</span>
                                            </div>
                                    ...
                                    <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                        ...
                                        >
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[11px] text-slate-400 font-medium">Single response for everyone</p>
                                            </div>
                                        ...
                                        >
                                            ...
                                            <div>
                                                <p className={cn("text-sm font-semibold", replyType === "filter" ? "text-slate-900" : "text-slate-500")}>Intelligence Filter</p>
                                                <p className="text-[11px] text-slate-400 font-medium">Keyword based logic</p>
                                            </div>
                                            <div className="ml-auto">
                                            ...
                                            {replyType === "generic" ? (
                                                    ...
                                                    <Field label="Message for Comment Reply" required icon={MessageCircle}>
                                                        <div className="relative border border-slate-200 rounded-xl p-4 focus-within:border-pink-400 transition-all bg-white">
                                                            <textarea
                                                                ...
                                                                placeholder="Type your generic response here..."
                                                ...
                                                <motion.div key="filter" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                                                        ...
                                                        <div key={rule.id} className="bg-white p-7 rounded-2xl border border-slate-200 shadow-sm space-y-8 relative group">
                                                            <div className="flex items-center justify-between border-b border-slate-50 pb-4">
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <h4 className="text-[13px] font-semibold text-slate-700">Filter Layer Rule</h4>
                                                                </div>
                                                            ...
                                                            <div className="flex gap-6 px-1">
                                                                <div className="flex items-center gap-2 cursor-pointer" onClick={() => updateRule(rule.id, { match_type: "contains" })}>
                                                                    ...
                                                                    <span className={cn("text-[12px] font-medium", rule.match_type === "contains" ? "text-slate-900" : "text-slate-400")}>Contains word</span>
                                                                </div>
                                                                <div className="flex items-center gap-2 cursor-pointer" onClick={() => updateRule(rule.id, { match_type: "exact" })}>
                                                                    ...
                                                                    <span className={cn("text-[12px] font-medium", rule.match_type === "exact" ? "text-slate-900" : "text-slate-400")}>Exact match</span>
                                                                </div>
                                                            </div>

                                                            <div className="grid grid-cols-1 gap-8">
                                                                <Field label="Filter Keyword(s)" required icon={Search}>
                                                                    <input
                                                                        type="text"
                                                                        value={rule.keywords}
                                                                ...
                                                                <Field label="Message for Comment Reply" required icon={MessageCircle}>
                                                                    <div className="relative border border-slate-200 rounded-xl p-4 focus-within:border-pink-400 transition-all bg-white">
                                                                        <textarea
                                                                            ...
                                                                            placeholder="Type matching response here..."
                                                                            className="w-full outline-none font-medium text-[14px] text-slate-700 resize-none h-[100px]"
                                                                        />
                                                                    </div>
                                                                </Field>
                                                                <Field label="Private Reply Template (Rule Based)" icon={Settings}>
                                                                    <div className="relative">
                                                                        <select
                                                    ...
                                                    <button onClick={addRule} className="w-full py-4 rounded-xl border-2 border-dashed border-pink-200 text-pink-600 font-semibold text-[13px] hover:bg-pink-50 transition-all flex items-center justify-center gap-2 group">
                                                        <Plus size={18} className="group-hover:rotate-90 transition-transform" /> Add another filter rule
                                                    </button>
                                                    ...
                                                    <div className="bg-slate-50/5 p-8 rounded-2xl border-2 border-dashed border-slate-100 space-y-8 mt-10">
                                                        ...
                                                        <Field label="Default Private Preset" icon={Settings}>
                                                            <div className="relative">
                                                                <select
                                                                    value={privateTemplateId || ""}
                                                                    onChange={e => setPrivateTemplateId(e.target.value || null)}
