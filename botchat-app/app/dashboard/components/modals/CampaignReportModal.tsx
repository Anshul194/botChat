    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-0 sm:p-4">
            ...
            <motion.div
                ...
                className="bg-[var(--card)] border border-[var(--border)] rounded-none sm:rounded-2xl w-full max-w-4xl shadow-2xl relative z-10 overflow-hidden flex flex-col h-[100dvh] sm:h-auto sm:max-h-[90vh]"
            >
                {/* Header */}
                <div className="p-4 sm:p-8 border-b border-[var(--border)] flex items-center justify-between bg-[var(--muted)]/50">
                    <div className="flex items-center gap-3 sm:gap-5 min-w-0">
                        ...
                        <div className="min-w-0">
                            <h2 className="text-base sm:text-xl font-black text-[var(--foreground)] uppercase tracking-tight leading-none truncate">{config.title}</h2>
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={fetchReport} className="p-3.5 rounded-2xl hover:bg-[var(--muted)] text-[var(--foreground)]/50 transition-all active:scale-90">
                            <RefreshCw className={cn("w-5 h-5", isLoading && "animate-spin")} />
                        </button>
                        <button onClick={onClose} className="p-3.5 rounded-2xl hover:bg-primary/10 text-[var(--foreground)]/50 hover:text-primary transition-all active:scale-90">
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                </div>
                ...
                {stats && (
                    <div className="px-6 py-4 sm:px-8 sm:py-6 bg-[var(--card)] border-b border-[var(--border)] grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6">
                        {[
                            { label: "Total Hits", value: stats.total },
                            { label: "Successful", value: stats.sent },
                            { label: "Failed", value: stats.failed },
                            ...(stats.hidden !== undefined ? [{ label: "Hidden", value: stats.hidden }] : [])
                        ].map((stat, i) => (
                            <div key={i} className="p-3.5 sm:p-4 rounded-2xl sm:rounded-3xl bg-primary/5 border border-primary/10">
                                <p className="text-[10px] font-black text-[var(--foreground)]/50 uppercase tracking-widest mb-1">{stat.label}</p>
                                <h4 className="text-2xl sm:text-3xl font-black tracking-tighter text-[var(--foreground)]">{stat.value}</h4>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {/* Table Header */}
                <div className="hidden sm:grid grid-cols-12 gap-4 px-10 py-4 bg-[var(--muted)]/50 border-b border-[var(--border)] text-[10px] font-black text-[var(--foreground)]/50 uppercase tracking-widest items-center">
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 custom-scrollbar bg-[var(--card)] text-sm">
                    {isLoading && data.length === 0 ? (
                        ...
                        data.map((item, idx) => (
                            <div key={idx} className="space-y-3">
                                <div className={cn(
                                    "grid grid-cols-12 gap-x-4 gap-y-5 px-5 py-5 sm:px-8 sm:py-6 rounded-2xl bg-[var(--card)] border transition-all items-center hover:shadow-xl hover:shadow-primary/5 cursor-pointer",
                                    expandedItem === item.id ? "border-primary/30 ring-4 ring-primary/5" : "border-[var(--border)]"
                                    ...
                                    <div className="col-span-11 sm:col-span-4 flex items-center gap-4">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-[14px] font-black text-[var(--foreground)] truncate leading-tight mb-1">
                                                {item.name || item.campaign_name || "Untitled Campaign"}
                                            </p>
                                            <div className="flex items-center gap-2 text-[10px] text-[var(--foreground)]/50 font-bold uppercase tracking-tight">
                                    ...
                                    <div className="col-span-6 sm:col-span-3 flex flex-col justify-center">
                                        <span className="text-[9px] font-bold text-[var(--foreground)]/50 uppercase tracking-wider mb-1 block sm:hidden">Configuration Logic</span>
                                        ...
                                        <div className="flex items-center gap-3 mt-2 pl-1">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[8px] font-bold text-[var(--foreground)]/50 uppercase tracking-widest mt-1">Multi</span>
                                            </div>
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[8px] font-bold text-[var(--foreground)]/50 uppercase tracking-widest mt-1">Inline</span>
                                            </div>
                                        </div>
                                    </div>
  
                                    <div className="col-span-5 sm:col-span-3 flex flex-col justify-center">
                                        <span className="text-[9px] font-bold text-[var(--foreground)]/50 uppercase tracking-wider mb-1 block sm:hidden">Status Tracking</span>
                                        <div className={cn(
                                            ...
                                            (item.is_active == 1 || item.is_active === "1" || item.status === 'active')
                                                ...
                                                : "bg-[var(--muted)] text-[var(--foreground)]/50 border border-[var(--border)]"
                                        )}>
                                        ...
                                        <div className="flex items-center gap-2 mt-2 ml-1 text-[var(--foreground)]/50 text-[10px] font-bold uppercase tracking-widest">
                                            ...
                                            <div className="mx-0 sm:mx-8 p-4 sm:p-8 rounded-2xl bg-primary/[0.02] border border-primary/10 space-y-4 sm:space-y-8">
                                                ...
                                                {(item.log_total !== undefined) && (
                                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                                                        {[
                                                            ...
                                                            <div key={i} className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-primary/5 border border-primary/10 text-center">
                                                                <p className="text-[9px] font-black text-[var(--foreground)]/50 uppercase tracking-widest mb-1">{s.label}</p>
                                                                <h5 className="text-lg sm:text-xl font-black text-[var(--foreground)] tracking-tighter">{s.value ?? 0}</h5>
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="p-6 rounded-3xl bg-[var(--card)] border border-primary/10 text-[13px] text-[var(--foreground)]/70 font-medium leading-relaxed italic shadow-sm">
                                                            {item.message ? `"${item.message}"` : "Dynamic filter-based content deployment"}
                                                        </div>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className="p-6 rounded-3xl bg-[var(--card)] border border-primary/10 shadow-sm flex flex-col justify-center">
                                                            <p className="text-[9px] font-black text-primary/40 uppercase tracking-widest mb-2">Privacy Control</p>
                                                            <div className={cn("text-[11px] font-black", (item.hide_after_reply == 1 || item.hide_after_reply === "1") ? "text-primary" : "text-[var(--foreground)]/50")}>
                                                        ...
                                                        <div className="p-6 rounded-3xl bg-[var(--card)] border border-primary/10 shadow-sm flex flex-col justify-center">
                                                            ...
                                                            <div className={cn("text-[11px] font-black", (item.is_template == 1 || item.is_template === "1") ? "text-primary" : "text-[var(--foreground)]/50")}>
                        ...
                        <div className="h-full flex flex-col items-center justify-center py-20 text-center space-y-6">
                            ...
                            <div>
                                <h3 className="text-lg font-black text-[var(--foreground)] uppercase tracking-[0.2em] leading-none">Insights Pending</h3>
                                <p className="text-[11px] font-bold text-[var(--foreground)]/50 uppercase tracking-tight mt-4 max-w-[240px] mx-auto">
                ...
                {/* Footer */}
                <div className="p-4 sm:p-6 bg-[var(--muted)]/50 border-t border-[var(--border)] flex justify-start">
                    <button 
                        onClick={onClose}
                        className="px-6 sm:px-8 py-3 rounded-xl sm:rounded-2xl border border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--muted)] font-bold text-[13px] transition-all flex items-center justify-center gap-1.5 active:scale-95 bg-[var(--card)]"
                    >
