...
function Toggle({ label, active, onClick }:{ label:string; active:boolean; onClick:()=>void }){
  return (
    <div className="flex items-center gap-4 group">
      <span className="text-[12px] font-bold text-slate-500 group-hover:text-slate-700 transition-colors ">{label}</span>
      <div className="flex items-center gap-3">
        <button onClick={onClick} className={cn(
          "w-11 h-5 rounded-full relative transition-all duration-200",
          active ? "bg-pink-600" : "bg-slate-300 shadow-inner"
        )}>
          <div className={cn("absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all", active ? "left-6.5" : "left-0.5")}/>
        </button>
        <span className="text-[10px] font-semibold text-slate-400 w-8 text-center ">{active ? "YES" : "NO"}</span>
      </div>
    </div>
  );
}
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-[#FDFDFF]">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-6">
            <div className="flex items-center gap-2 mb-2">
               ...
               <h3 className="text-[12px] font-semibold text-rose-500 ">Offensive Comments Settings</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="space-y-2">
                 <label className="text-[11px] font-bold text-slate-500 ">Write down the offensive keywords (comma separated)</label>
                 <div className="relative">
               ...
               <div className="space-y-5">
                 <div className="flex items-center justify-between px-1">
                    <label className="text-[11px] font-bold text-slate-500 ">Select a message template for private reply</label>
                    <div className="flex gap-3 text-[10px] font-extrabold text-pink-600">
          ...
          <div className="bg-white border border-slate-100 rounded-[22px] p-6 shadow-xs space-y-5">
             <div className="flex items-center gap-4 cursor-pointer group" onClick={()=>setForm({...form, message_type:"generic"})}>
                ...
                <span className={cn("text-[11px] font-semibold", form.message_type === "generic" ? "text-pink-600":"text-slate-400")}>GENERIC MESSAGE FOR ALL</span>
             </div>
             <div className="flex items-center gap-4 cursor-pointer group" onClick={()=>setForm({...form, message_type:"filtered"})}>
                ...
                <span className={cn("text-[11px] font-semibold", form.message_type === "filtered" ? "text-pink-600":"text-slate-400")}>SEND MESSAGE BY FILTERING WORD/SENTENCE</span>
             </div>
          </div>
                       ...
                       <div className="flex items-center justify-between">
                          <div className="flex items-center gap-6">
                             <div className="flex items-center gap-3 cursor-pointer" onClick={()=>setFilterRules(filterRules.map(r=>r.id===rule.id ? {...r, match_type:"exact"}:r))}>
                               ...
                               <span className={cn("text-[9px] font-medium", rule.match_type === "exact" ? "text-slate-800":"text-slate-400")}>REPLY IF THE FILTER WORD EXACTLY MATCHES</span>
                             </div>
                             <div className="flex items-center gap-3 cursor-pointer" onClick={()=>setFilterRules(filterRules.map(r=>r.id===rule.id ? {...r, match_type:"any"}:r))}>
                               <div className={cn("w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all", rule.match_type === "any" ? "border-pink-600 bg-pink-600":"border-slate-300")}>
                                  {rule.match_type === "any" && <div className="w-1.5 h-1.5 rounded-full bg-white"/>}
                               </div>
                               <span className={cn("text-[9px] font-medium", rule.match_type === "any" ? "text-slate-800":"text-slate-400")}>REPLY IF ANY MATCHES OCCURS WITH FILTER WORD</span>
                             </div>
                          </div>
