...
export default function SearchBox({ onOpenFilters, activeTab, setActiveTab }: SearchBoxProps) {
    ...
    return (
        <div className="space-y-3">
            {/* ── Filter Tab Bar ── */}
            <div className="flex items-center gap-0.5 sm:gap-1 p-0.5 sm:p-1 rounded-xl bg-muted/50 border border-border/40">
                {TABS.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "relative flex-1 flex items-center justify-center gap-0.5 sm:gap-1 py-1.5 sm:py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all duration-200 outline-none min-w-0",
                                isActive
                                    ? "text-white"
